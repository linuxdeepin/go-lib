...
func (g *Graphic) TestFillImageCache(c *C) {
	resultFile, useCache, err := FillImageCache(originImg, 1024, 768, FillTile, FormatPng)
	if err != nil {
		c.Error(err)
	}
...
func (g *Graphic) TestIsSupportedImage(c *C) {
	c.Check(IsSupportedImage(originImg), Equals, true)
...
func (g *Graphic) TestThumbnailImageCache(c *C) {
	resultFile, useCache, err := ThumbnailImageCache(originImg, 200, 200, FormatPng)
	if err != nil {
		c.Error(err)
	}
